...
********************* 3) Robust Variances for Iterated GMM Estimations  ************************************************ 
	...
	if  "`vce'" == "iter" {
		...
		*mata: mata set matafavor speed, perm
		*qui xtabond2 `varlist' `if' `in', svmat noleveleq `options' two robust
        mata: se_iter("`b'", "`N'", "`DC_V'", "`V'", "`Wind_V'" )
    }
...
*	ereturn post `b' `V_diag', esample(`touse') buildfvinfo depname(`depvarname')
	...
	matrix colnames `DC_V' = `xvars_nocons'
    matrix rownames `DC_V' = `xvars_nocons'
	ereturn post `b' `DC_V', esample(`touse') buildfvinfo depname(`depvarname')
	
	ereturn matrix V_iter = `V'
	ereturn matrix V_wind = `Wind_V'
...
void se_iter( string scalar bname,  string scalar N_g_name,  string scalar DC_Vname,  string scalar Vname, string scalar Wind_Vname)

{
	...
	V_iter = H
	V_wind = (H_iter_inv * invsym((1/N^2) * XpZ*invsym(Omega_iter)*(XpZ')) * H_iter_inv')/N

	
	st_matrix(bname, b_iter')
    st_numscalar(N_g_name, N)
	
    st_matrix(DC_Vname, V_dc_iter)
	st_matrix(Vname, V_iter)
	st_matrix(Wind_Vname, V_wind)
