*! dcivreg version 1.0 - Last update: June 11th, 2019 
...
void se_one( string scalar bname,  string scalar Gname,  string scalar Vname)

{
    ...
    real scalar    n, kx, kz, N, temp_dim
	...
	temp_psi_one_2g = temp_X_g'*temp_Z_g*invsym(W_hat)*temp_Z_g'*temp_e_one_g
...
void se_two( string scalar bname,  string scalar Gname,  string scalar Vname)

{
	...
	temp_psi_two_2g = temp_X_g'*temp_Z_g*invsym(Omega_one)*temp_Z_g'*temp_e_two_g
	temp_psi_two_3g = -(1/N)*XpZ*invsym(Omega_one)*(temp_Z_g'*temp_e_two_g)*(temp_e_two_g'*temp_Z_g)*invsym(Omega_one)*Z'*e_two
...
void se_iter( string scalar bname,  string scalar Gname,  string scalar Vname,  string scalar Vdname)

{
	...
	H_iter = (1/(N^2))*(XpZ*invsym(Omega_iter)*(XpZ')) - (1/(N^3))*(XpZ*invsym(Omega_iter))*temp_H_iter
	V_dc_iter = pinv(H_iter)*(temp_psi_iter_sq/N)*pinv(H_iter')
