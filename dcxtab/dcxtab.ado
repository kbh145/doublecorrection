*! dcivreg version 1.20 - Last update: June 29th, 2019
 ...
 * For the return of proper error messages
	...
	if  "`vce'" == "one" {
	qui xtabond2 `varlist' `if' `in', svmat noleveleq `options' robust 
	}
...
********************* 2) Robust Variances for (Two-step) GMM Estimations  ************************************************ 
	...
	*	mata: mata set matafavor speed, perm
	*	qui xtabond2 `varlist' `if' `in', svmat noleveleq `options' twostep 
        mata: se_two("`b'", "`N'", "`V'")
    }
...
*	ereturn post `b' `V_diag', esample(`touse') buildfvinfo depname(`depvarname')
	...
	ereturn matrix V_iter = V_iter
	ereturn matrix V_wind = V_wind
...

{
	...
	V_iter = H*N
	V_wind = (H_iter_inv * ((1/N^2) * XpZ*invsym(Omega_iter)*(XpZ')) * H_iter_inv')/N
	...
	st_matrix("V_iter", V_iter)
	st_matrix("V_wind", V_wind)
